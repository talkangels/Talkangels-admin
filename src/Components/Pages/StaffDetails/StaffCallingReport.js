...
import { FetchAllStaffCallRecodes, FetchAllStaffCallRecodesSummary } from '../../services/dataReport';
...
const StaffCallingReport = () => {
    ...
    const handleFetchCallRecods = async () => {
        try {
            setLoading(true)
            const result = await FetchAllStaffCallRecodes(id, page, limit, mobile_number, selected, from, to)
    ...
    const HnadleFetchAllStaffCallRecodesSummary = async () => {
        try {
            const result = await FetchAllStaffCallRecodesSummary(id, mobile_number, selected, from, to)
    ...
    useEffect(() => {
        if (!mobile_number && !selected && !from && !to) {
            ...
            HnadleFetchAllStaffCallRecodesSummary();
            return;
        }
    ...
    return (
        <>
            {loading && <Spinner />}
            <FaArrowLeftLong className='text-white text-[30px] cursor-pointer' onClick={() => navigate(Routing.StaffDetails)} />
...
export default StaffCallingReport
