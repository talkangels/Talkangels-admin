...
const WebPageAdmin = () => {
    ...
    const addTab = (tabName) => {
        ...
        if (!isNewTabPresent) {
            setTabs(prevTabs => [...prevTabs, { name: tabName, isActive: false }]);
        }
    };
                        ...
                        <div className="grid grid-cols-2 items-center justify-between gap-5 mb-5">
                            {
                                tabs.map((item, i) => (
                                    <button
                                        key={item.name}
                                        className={`w-full h-[55px] rounded-full text-lg font-semibold flex items-center justify-center ${item.isActive ? "bg-Sky text-white" : "bg-transparent border-[#88888E] border-[0.5px] text-[#88888E]"
                                            }`}
