...
export default function TermsConditionsAdmin({ termsConditions, setTermsConditions }) {
    ...
    const addTopic = (index) => {
        const newData = [...termsConditions.data];
    ...
    return (
        <>
            {
                termsConditions.data.map((section, sectionIndex) => (
                    <div className='flex gap-4 py-5 border-b border-white/25 '>
                        ...
                        {
                            sectionIndex === termsConditions.data.length - 1 ?
                                <button
                                    onClick={addField}
